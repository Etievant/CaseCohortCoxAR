...
Onerun <- function (p) {
  ...
  myfile  <- paste0("SimulationResults_stratified-n", n, "-Prob.y1", Prob.y1, 
...
for (p in 1:nrow(parameters)) {
  ...
  load(paste0("SimulationResults_stratified-n", n, "-Prob.y1", Prob.y1, 
              "-Prob.y2", Prob.y2, "-K", K, "Part", part, ".Rdata"))
  RES <- rbind(RES, res)
...
myfile  <- paste0("SimulationResults_stratified.Rdata")
...
a <- cbind(details.logAR.y1[, c(84, 85)], rep(1,12), details.logAR.y1[, c(83, 90, 91, 92)]) 
a[,3:7] <- data.frame(lapply(a[,3:7] , function(x) format(x, digits = 1)))
