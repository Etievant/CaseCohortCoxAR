...
##              - sampling of only a fraction of the cases with an event of 
##              interest (y1) and only a fraction of the cases with a competing 
##              event (y2)
##              - a decreasing/increasing hazard, with different Weibull 
##              parameters for y1 and y2
##              - design weights 
##              - calibrated weights  
##
...
Nreplic <- 5000 / 20 # we break down the replications in 20

set.seed(12345)
seed <- round(abs(rnorm(20) * 10^5))
...
Onerun <- function (p) {
  ...
  for (nrep in 1:Nreplic) {
    ...
    U               <- runif(n)
    death.times     <- (-log(U) / (0.02 / 10)) # 2% censoring by death at 10 years
    ...
    prob = ((shape.y1 * y.times^(shape.y1 - 1) * bh.y1 * exp(beta11 * X11 + beta21 * X21 + beta31 * X31)) / (shape.y1 * y.times^(shape.y1 - 1) * bh.y1 * exp(beta11 * X11 + beta21 * X21 + beta31 * X31) + shape.y2 * y.times^(shape.y2 - 1) * bh.y2 * exp(beta12 * X12 + beta22 * X22 + beta32 * X32)))
    ...
    cohort    <- cbind(id = 1:n, X11, X21, X31, X12, X22, X32, X11.proxy, 
                       ...
                       status2, n, n.strata = n.strata[status + 1], 
                       n.y1 = n.y1, n.y2 = n.y2)
    ...
    cohort          <- cbind(cohort, m.strata = m.strata[status + 1])
    ...
    # --------------------------------------------------------------------------
    # At risk indicator matrix and counting process matrix for the phase-one 
    # data at all event times, even for cases not in phase-two -----------------
    ...
    est.y1.casecohort <- influences.generalized(mod = mod.casecohort.y1, x = c(-1, 1, -0.6),
                                                t1 = 0, t2 = 8, 
                                                Breslow.all = TRUE,
                                                riskmat.phase1 = riskmat.phase1.y1, 
                                                dNt.phase1 = dNt.phase1.y1)
    
    est.y2.casecohort <- influences.generalized(mod = mod.casecohort.y2, x = c(0.5, 1), 
    ...
    robvar.Lambda0.t12.y2.hat.casecohort   <- robustvariance(est.y2.casecohort$infl1.Lambda0.t12[,n.events.y2] + est.y2.casecohort$infl2.Lambda0.t12[,n.events.y2]) # equal to diag(mod.casecohort.y2$var)
    var.Lambda0.t12.y2.hat.casecohort      <- variance.generalized(casecohort = casecohort, 
                                                                   infl1 = as.matrix(est.y2.casecohort$infl1.Lambda0.t12[,n.events.y2]), 
                                                                   infl2 = as.matrix(est.y2.casecohort$infl2.Lambda0.t12[,n.events.y2]), 
                                                                   cohort = cohort, 
                                                                   Breslow.all = TRUE) 
    ...
    # --------------------------------------------------------------------------
    # Estimation using the case-cohort with design weights and when the 
    # numerator of the Breslow estimator is weighted ---------------------------
    
    est.y1.casecohort2 <- influences(mod = mod.casecohort.y1, x = c(-1, 1, -0.6),
                                     t1 = 0, t2 = 8)
    
    est.y2.casecohort2 <- influences(mod = mod.casecohort.y2, x = c(0.5, 1), 
                                     t1 = 0, t2 = 8, competing = TRUE, 
    ...
    est.y2.casecohort2 <- influences(mod = mod.casecohort.y2, x = c(0.5, 1), 
                                     t1 = 0, t2 = 8)
    n.events.y2 <- length(est.y2.casecohort2$Lambda0.t12.hat)
    Lambda0.t12.y2.hat.casecohort2 <- est.y2.casecohort2$Lambda0.t12.hat[n.events.y2]
    robvar.Lambda0.t12.y2.hat.casecohort2   <- robustvariance(est.y2.casecohort2$infl.Lambda0.t12[,n.events.y2]) # equal to diag(mod.casecohort.y2$var)
    var.Lambda0.t12.y2.hat.casecohort2      <- variance.generalized(casecohort = casecohort, 
                                                                    infl = as.matrix(est.y2.casecohort2$infl.Lambda0.t12[,ncol(est.y2.casecohort2$infl.Lambda0.t12)]), 
                                                                    cohort = cohort, 
                                                                    Breslow.all = FALSE) 
    
    res.casecohort2.rob <- cbind("CaseCohort2 Robust", t(beta.y1.hat.casecohort), 
                                 t(beta.y2.hat.casecohort), 
                                 Lambda0.t12.y2.hat.casecohort2,
                                 AR.y1.hat.casecohort2, 
                                 PR.y1.hat.casecohort2, 
                                 t(robvar.beta.y1.hat.casecohort2),
                                 t(robvar.beta.y2.hat.casecohort2),
                                 robvar.Lambda0.t12.y2.hat.casecohort2,
                                 robvar.AR.y1.hat.casecohort2, 
                                 robvar.PR.y1.hat.casecohort2)
    
    res.casecohort2 <- cbind("CaseCohort2", t(beta.y1.hat.casecohort), 
                             t(beta.y2.hat.casecohort), 
                             Lambda0.t12.y2.hat.casecohort2,
                             AR.y1.hat.casecohort2, 
                             PR.y1.hat.casecohort2, t(var.beta.y1.hat.casecohort2),
                             t(var.beta.y2.hat.casecohort2),
                             var.Lambda0.t12.y2.hat.casecohort2,
                             var.AR.y1.hat.casecohort2, var.PR.y1.hat.casecohort2)
    
    # --------------------------------------------------------------------------
    # Estimation using the case-cohort with calibrated weights -----------------
    
    # --------------------------------------------------------------------------
    # When the numerator of the Breslow estimator is not weighted --------------
    ...
    est.y1.calib <- influences.generalized.calib(mod = mod.calib.y1, A = A,
                                                 x = c(-1, 1, -0.6),
                                                 t1 = 0, t2 = 8, 
                                                 Breslow.all = TRUE,
                                                 riskmat.phase1 = riskmat.phase1.y1, 
    ...
    est.y2.calib <- influences.generalized.calib(mod = mod.calib.y2, A = A,
                                                 x = c(0.5, 1),
                                                 t1 = 0, t2 = 8, 
                                                 Breslow.all = TRUE,
                                                 riskmat.phase1 = riskmat.phase1.y2, 
                                                 dNt.phase1 = dNt.phase1.y2,
                                                 competing = TRUE, 
                                                 event.times.y1 = est.y1.calib$event.times)
    
    est.risk.calib <- risk.influences.generalized(est.y1.calib, est.y2.calib)
    ...
    var.beta.y1.hat.calib      <- variance.generalized(casecohort = casecohort, 
                                                       infl1 = est.y1.calib$infl1.beta, 
                                                       infl2 = est.y1.calib$infl2.beta, 
                                                       cohort = cohort, 
                                                       Breslow.all = TRUE, 
                                                       calibrated = TRUE) 
    var.beta.y2.hat.calib      <- variance.generalized(casecohort = casecohort, 
                                                       infl1 = est.y2.calib$infl1.beta, 
                                                       infl2 = est.y2.calib$infl2.beta, 
                                                       cohort = cohort, 
                                                       Breslow.all = TRUE, 
                                                       calibrated = TRUE) 
    var.AR.y1.hat.calib        <- variance.generalized(casecohort = casecohort, 
                                                       infl1 = est.risk.calib$infl1.AR.y1, 
                                                       infl2 = est.risk.calib$infl2.AR.y1, 
                                                       cohort = cohort, 
                                                       Breslow.all = TRUE, 
                                                       calibrated = TRUE) 
    var.PR.y1.hat.calib        <- variance.generalized(casecohort = casecohort, 
                                                       infl1 = est.risk.calib$infl1.PR.y1, 
                                                       infl2 = est.risk.calib$infl2.PR.y1, 
                                                       cohort = cohort, 
                                                       Breslow.all = TRUE, 
                                                       calibrated = TRUE) 
    ...
    var.Lambda0.t12.y2.hat.calib      <- variance.generalized(casecohort = casecohort, 
                                                              infl1 = as.matrix(est.y2.calib$infl1.Lambda0.t12[,n.events.y2]),
                                                              infl2 = as.matrix(est.y2.calib$infl2.Lambda0.t12[,n.events.y2]),
                                                              cohort = cohort, 
                                                              Breslow.all = TRUE, 
                                                              calibrated = TRUE) 
    ...
    est.y1.calib22 <- influences.generalized.calib(mod = mod.calib.y1, A = A,
                                                   x = c(-1, 1, -0.6),
                                                   t1 = 0, t2 = 8, 
                                                   fraction.y = TRUE,
                                                   riskmat.phase1 = riskmat.phase1.y1, 
    ...
    est.y2.calib22 <- influences.generalized.calib(mod = mod.calib.y2, A = A,
                                                   x = c(0.5, 1),
                                                   t1 = 0, t2 = 8, 
                                                   fraction.y = TRUE,
                                                   riskmat.phase1 = riskmat.phase1.y2, 
                                                   dNt.phase1 = dNt.phase1.y2,
                                                   competing = TRUE, 
                                                   event.times.y1 = est.y1.calib22$event.times,
                                                   Breslow.all = FALSE)
    
    est.risk.calib22 <- risk.influences.generalized(est.y1.calib22, est.y2.calib22)
    
    beta.y1.hat.calib22 <- mod.calib.y1$coefficients
    beta.y2.hat.calib22 <- mod.calib.y2$coefficients
    
    AR.y1.hat.calib22 <- est.risk.calib22$AR.y1
    PR.y1.hat.calib22 <- est.risk.calib22$PR.y1
    
    robvar.beta.y1.hat.calib22   <- robustvariance(est.y1.calib22$infl1.beta + est.y1.calib22$infl2.beta) 
    robvar.beta.y2.hat.calib22   <- robustvariance(est.y2.calib22$infl1.beta + est.y2.calib22$infl2.beta) 
    robvar.AR.y1.hat.calib22     <- robustvariance(est.risk.calib22$infl1.AR.y1 + est.risk.calib22$infl2.AR.y1)
    robvar.PR.y1.hat.calib22     <- robustvariance(est.risk.calib22$infl1.PR.y1 + est.risk.calib22$infl2.PR.y1)
    
    var.beta.y1.hat.calib22      <- variance.generalized(casecohort = casecohort, 
                                                         infl1 = est.y1.calib22$infl1.beta, 
                                                         infl2 = est.y1.calib22$infl2.beta, 
                                                         cohort = cohort, 
                                                         Breslow.all = FALSE,
                                                         calibrated = TRUE) 
    var.beta.y2.hat.calib22      <- variance.generalized(casecohort = casecohort, 
                                                         infl1 = est.y2.calib22$infl1.beta, 
                                                         infl2 = est.y2.calib22$infl2.beta, 
                                                         cohort = cohort, 
                                                         Breslow.all = FALSE,
                                                         calibrated = TRUE) 
    var.AR.y1.hat.calib22        <- variance.generalized(casecohort = casecohort, 
                                                         infl1 = est.risk.calib22$infl1.AR.y1, 
                                                         infl2 = est.risk.calib22$infl2.AR.y1, 
                                                         cohort = cohort, 
                                                         Breslow.all = FALSE,
                                                         calibrated = TRUE) 
    var.PR.y1.hat.calib22        <- variance.generalized(casecohort = casecohort, 
                                                         infl1 = est.risk.calib22$infl1.PR.y1, 
                                                         infl2 = est.risk.calib22$infl2.PR.y1, 
                                                         cohort = cohort, 
                                                         Breslow.all = FALSE,
                                                         calibrated = TRUE) 
    ...
    est.y2.calib22 <- influences.generalized.calib(mod = mod.calib.y2, A = A,
                                                   ...
                                                   dNt.phase1 = dNt.phase1.y2,
                                                   Breslow.all = FALSE)
    n.events.y2 <- length(est.y2.calib22$Lambda0.t12.hat)
    Lambda0.t12.y2.hat.calib22 <- est.y2.calib22$Lambda0.t12.hat[n.events.y2]
    robvar.Lambda0.t12.y2.hat.calib22   <- robustvariance(est.y2.calib22$infl1.Lambda0.t12[,n.events.y2] + est.y2.calib22$infl2.Lambda0.t12[,n.events.y2]) 
    var.Lambda0.t12.y2.hat.calib22      <- variance.generalized(casecohort = casecohort, 
                                                                infl1 = as.matrix(est.y2.calib22$infl1.Lambda0.t12[,n.events.y2]),
                                                                infl2 = as.matrix(est.y2.calib22$infl2.Lambda0.t12[,n.events.y2]),
                                                                cohort = cohort, 
                                                                Breslow.all = FALSE,
                                                                calibrated = TRUE) 
    
    res.calib22.rob <- cbind("CaseCohort2 Robust Calibration", t(beta.y1.hat.calib22), 
                             t(beta.y2.hat.calib22), 
                             Lambda0.t12.y2.hat.calib22,
                             AR.y1.hat.calib22, 
                             PR.y1.hat.calib22, 
                             t(robvar.beta.y1.hat.calib22),
                             t(robvar.beta.y2.hat.calib22),
                             robvar.Lambda0.t12.y2.hat.calib22,
                             robvar.AR.y1.hat.calib22, 
                             robvar.PR.y1.hat.calib22)
    
    res.calib22 <- cbind("CaseCohort2 Calibration", t(beta.y1.hat.calib22), 
                         t(beta.y2.hat.calib22), 
                         Lambda0.t12.y2.hat.calib22,
                         AR.y1.hat.calib22, 
                         PR.y1.hat.calib22, t(var.beta.y1.hat.calib22),
                         t(var.beta.y2.hat.calib22),
                         var.Lambda0.t12.y2.hat.calib22,
                         var.AR.y1.hat.calib22, var.PR.y1.hat.calib22)
    
    
    recap <- rbind(res.casecohort, res.casecohort.rob, res.casecohort2, 
                   res.casecohort2.rob, res.calib, res.calib.rob,
                   res.calib22, res.calib22.rob, res.cohort)
    ...
    res <- rbind(res, cbind(recap, n, K, n.y0 = n.strata[1], 
                            ...
                            m.y0 = m.strata[1], m.y1 = m.strata[2], 
                            m.y2 = m.strata[3], ncasecohort = nrow(casecohort), 
                            Prob.y1, Prob.y2, beta11, beta21, beta31, beta12, 
                            beta32, AR.y1, PR.y1))
    row.names(res) <- NULL
  }
  myfile  <- paste0("SimulationResults_stratified_fraction_Weibull-n", n, 
                    "-Prob.y1", Prob.y1, "-Prob.y2", Prob.y2, "-K", K, "Part", 
                    part, ".Rdata")
...
for (p in 1:nrow(parameters)) {
  ...
  load(paste0("SimulationResults_stratified_fraction_Weibull-n", n, "-Prob.y1", 
              Prob.y1, "-Prob.y2", Prob.y2, "-K", K, "Part", part, ".Rdata"))
  RES <- rbind(RES, res)
  
}
RECAP           <- as.data.frame(RES)
ColNames        <- colnames(RECAP[,c(2:35)])
...
myfile  <- paste0("SimulationResults_stratified_fraction_Weibull.Rdata")
...
list.methods <- c("Cohort", "CaseCohort", "CaseCohort Robust", "CaseCohort2", 
                  "CaseCohort2 Robust", "CaseCohort Calibration", 
                  "CaseCohort Robust Calibration", "CaseCohort2 Calibration", 
                  "CaseCohort2 Robust Calibration")
list.methods.col <- c("Cohort", "CC", "CCRobust", "CC2", "CC2Robust",
                      "CCCalib", "CCCalibRobust", "CC2Calib", "CC2CalibRobust")
...
for (i in 1:nrow(parameters)) {
  ...
  m.y0 <- mean(RECAP1$m.y0)
  m.y1 <- mean(RECAP1$m.y1)
  m.y2 <- mean(RECAP1$m.y2)
...
simul.name = "_stratified_fraction_Weibull"

save(details.AR.y1, file = paste0("details.AR.y1", simul.name, ".Rdata"))
col.names.AR.y1 <- colnames(details.AR.y1[,c(1:92)])
...
col.names.PR.y1 <- colnames(details.PR.y1[,c(1:92)])
...
col.names.logAR.y1 <- colnames(details.logAR.y1[,c(1:92)])
...
details.logAR.y1[col.names.logAR.y1] <- round(details.logAR.y1[col.names.logAR.y1], 
                                              digits = 5)
details.logAR.y1[46:66] <- round(details.logAR.y1[46:66], digits = 3)
...
col.names.logPR.y1 <- colnames(details.logPR.y1[,c(1:92)])
...
## Print Latex tables ----------------------------------------------------------
### No calibration
a <- cbind(details.logAR.y1[details.logAR.y1$K == 0.5, c(84, 85, 80, 2, 20, 29:30, 47:48, 65, 4, 22, 31:32, 49:50, 67)]) # case cohort with K = 1, no calibration
b <- cbind(details.logAR.y1[details.logAR.y1$K == 0.8, c(84, 85, 80, 2, 20, 29:30, 47:48, 65, 4, 22, 31:32, 49:50, 67)]) # case cohort with K = 2, no calibration

a_scientific        <- a
a_scientific[,c(4:7,11:14)]  <- data.frame(lapply(a[,c(4:7,11:14)], function(x) format(x, scientific = TRUE, digits = 2)))
a_scientific[,c(10,17)]      <- data.frame(lapply(a[,c(10,17)], function(x) format(x, digits = 2)))
a                           <- data.frame(lapply(a_scientific , function(x) format(x, digits = 3)))
print(xtable(a), include.rownames=FALSE) # K = 1, no calibration

b_scientific        <- b 
b_scientific[,c(4:7,11:14)]  <- data.frame(lapply(b[,c(4:7,11:14)], function(x) format(x, scientific = TRUE, digits = 2)))
b_scientific[,c(10,17)]      <- data.frame(lapply(b[,c(10,17)], function(x) format(x, digits = 2)))
b                   <- data.frame(lapply(b_scientific , function(x) format(x, digits = 3)))
print(xtable(b), include.rownames=FALSE) # K = 2, no calibration

### Calibration
a <- cbind(details.logAR.y1[details.logAR.y1$K == 0.5, c(84, 85, 80, 6, 24, 33:34, 51:52, 69, 8, 26, 35:36, 53:54, 71)]) # case cohort with K = 1, no calibration
b <- cbind(details.logAR.y1[details.logAR.y1$K == 0.8, c(84, 85, 80, 6, 24, 33:34, 51:52, 69, 8, 26, 35:36, 53:54, 71)]) # case cohort with K = 2, no calibration
...
### Cohort
d <- cbind(details.logAR.y1[details.logAR.y1$K == 0.5, c(84, 85, 80, 1, 19, 28, 46)]) 
...
b <- cbind(details.logAR.y1[, c(90, 91, 92)]) 
