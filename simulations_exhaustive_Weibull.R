...
Onerun <- function (p) {
  ...
  for (nrep in 1:Nreplic) {
    ...
    U               <- runif(n)
    death.times     <- (-log(U) / (0.02 / 10)) # 2% censoring at 10 years
    ...
    cohort$weights <- 1
    cohort$weights[(cohort$status == 0)] <- n / m # 1 / sampling fraction
  ...
  myfile  <- paste0("SimulationResults_exhaustive_Weibull-n", n, "-Prob.y1", 
...
for (p in 1:nrow(parameters)) {
  ...
  load(paste0("SimulationResults_exhaustive_Weibull-n", n, "-Prob.y1", Prob.y1, 
...
myfile  <- paste0("Simulation_exhaustive_Weibull.Rdata")
