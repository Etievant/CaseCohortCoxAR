...
##              - sampling of all the cases with an event of interest (y1)
##              - sampling only a fraction of the cases with a competing 
##              event (y2)
...
Onerun <- function (p) {
  ...
  for (nrep in 1:Nreplic) {
    ...
    indiv.sampled <- c(indiv.sampled, cohort$id[cohort$status == 1])
  ...
  myfile  <- paste0("SimulationResults_stratified_Weibull-n", n, "-Prob.y1", 
...
for (p in 1:nrow(parameters)) {
  ...
  load(paste0("SimulationResults_stratified_Weibull-n", n, "-Prob.y1", 
...
myfile  <- paste0("SimulationResults_stratified_Weibull.Rdata")
...
simul.name = "_stratified_Weibull"
