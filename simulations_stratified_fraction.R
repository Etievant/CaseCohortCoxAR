...
Onerun <- function (p) {
  ...
  myfile  <- paste0("SimulationResults_stratified_fraction-n", n, "-Prob.y1", 
                    Prob.y1, "-Prob.y2", Prob.y2, "-K", K, "Part", part, 
                    ".Rdata")
...
for (p in 1:nrow(parameters)) {
  ...
  load(paste0("SimulationResults_stratified_fraction-n", n, "-Prob.y1", 
...
myfile  <- paste0("SimulationResults_stratified_fraction.Rdata")
...
simul.name = "stratified_fraction"
...
print(xtable(d), include.rownames=FALSE) # K = 1, no calibration

### Subcohort and case-cohort size
a <- cbind(details.logAR.y1[, c(84, 85, 83, 83, 90, 91, 92)]) 
a[,5:7] <- data.frame(lapply(a[,5:7] , function(x) format(x, digits = 1)))
